function getValoreRiga_parte1(riga){
	let primo, ultimo;
	let array = riga.match(/\d/g);
	if (array){
		primo = array[0];
		ultimo = array.slice(-1)
		return parseInt(primo)*10 + parseInt(ultimo);		
	}else{
		return 0;		
	}	
}

function getRisultato_parte1(){
	let ret = 0;	
	sample.split('\n').forEach(function(riga, index){
		ret += parseInt(getValoreRiga_parte1(riga));
		})
	return ret;
	//risultato esatto = 56042
}

function getValoreRiga_parte2(riga){
	let primo, ultimo;
	let ret = 0;	
	let array = Array.from(riga.matchAll(/(?=(one|two|three|four|five|six|seven|eight|nine|\d))/g));
	
	if(array && array.length > 0){		
		if(array.length == 1){
			primo = pGetNumeroFromString(array[0][1]);
			ultimo = primo
		}else{
			primo = pGetNumeroFromString(array[0][1]);
			ultimo = pGetNumeroFromString(array[array.length - 1].slice(-1));			
		}
		ret = primo*10 + ultimo;
	}
	return ret;
...
function getRisultato_parte2(elenco = ''){
	if (elenco == ''){elenco = sample}
	let ret = 0;	
	elenco.split('\n').forEach(function(riga, index){
		ret += parseInt(getValoreRiga_parte2(riga));		
	})
	return ret;
	//risultato esatto = 55358
}
var STRINGA_TO_NUMERO = {'one':1,'two':2,'three':3,
'four':4,'five':5,'six':6,
'seven':7,'eight':8,'nine':9}
function pGetNumeroFromString(s){
	if(/\d+/.test(s)){
		return parseInt(s);		
	}else{		
		let sRet = STRINGA_TO_NUMERO[s];
		if(/\d+/.test(sRet)){
			return sRet;
		}else{
			return 0;
...
var sample = `99lbqpxzzlbtvkmfrvrnmcxttseven
q7cnfslbtpkvseven
