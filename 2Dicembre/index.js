...
function pGetValoreRiga_1(riga){
	let ID;
	let arrayEstrazioni;
	let estrazioneRED = 0;
	let estrazioneGREEN = 0;
	let estrazioneBLUE = 0;
	//riga = 'Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green'
	ID = riga.match(/(?<=Game\s)(\d+)/)[0]
	arrayEstrazioni = riga.replace(/Game \d+\:/, '').split(';');
	//[' 3 blue, 4 red', ' 1 red, 2 green, 6 blue', ' 2 green']
	
	let r,g,b;
	arrayEstrazioni.forEach(function(estrazione, index){
		
		b = estrazione.match(/\d+(?=\sblue)/);
		if(b){
			estrazioneBLUE = Math.max(estrazioneBLUE, parseInt(b[0]))
		}
		
		r = estrazione.match(/\d+(?=\sred)/);
		if(r){
			estrazioneRED = Math.max(estrazioneRED, parseInt(r[0]))
		}
		
		g = estrazione.match(/\d+(?=\sgreen)/);
		if(g){
			estrazioneGREEN = Math.max(estrazioneGREEN, parseInt(g[0]))
		}			
	});
	
	return pValidaEstrazioni_1(ID, estrazioneRED, estrazioneGREEN, estrazioneBLUE)
	
}

function pValidaEstrazioni_1(ID, estrazioneRED, estrazioneGREEN, estrazioneBLUE){
	if(estrazioneRED <= RED && estrazioneGREEN <= GREEN && estrazioneBLUE <= BLUE){
		return parseInt(ID);
	}
	return 0;	
}

function GetRisultato_1(elenco = ''){	
	if (elenco == ''){elenco = sample}
	let ret = 0;	
	elenco.split('\n').forEach(function(riga, index){
		ret += parseInt(pGetValoreRiga_1(riga));		
	})
	return ret;
	//risultato esatto = 2476
}

/***parte 2*/

function pGetValoreRiga_2(riga){
	let ID;
	let arrayEstrazioni;
	let estrazioneRED = 0;
	let estrazioneGREEN = 0;
	...
	return estrazioneGREEN*estrazioneRED*estrazioneBLUE;
	
}

function GetRisultato_2(elenco = ''){	
	if (elenco == ''){elenco = sample}
	let ret = 0;	
	elenco.split('\n').forEach(function(riga, index){
		ret += parseInt(pGetValoreRiga_2(riga));		
	})
	return ret;
	//risultato esatto = 2476
}
...
/***SAMPLE*/
var sample = `Game 1: 7 blue, 4 red, 11 green; 2 red, 2 blue, 7 green; 2 red, 13 blue, 8 green; 18 blue, 7 green, 5 red
